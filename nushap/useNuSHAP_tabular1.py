...
if __name__ == '__main__':
    ds_md = [('connect_4', 'knn'), ('spambase', 'knn'), ('spectf', 'knn'),
    ...
    for (ds, md) in ds_md:
        ...
        if md == 'knn':
            ...
            my_nushap = NuSHAP_tabular(model.predict_proba, X_train, feature_names=list(train_df.iloc[:, :-1].columns),
                                       keep_index=True)
            explainer = shap.PermutationExplainer(model=my_nushap.predict_and_collect_samples, masker=X_train,
                                               feature_names=my_nushap.data_feature_names)
        elif md == 'xgboost':
            error = 0.0015
            alpha = 0.015
            model = xgboost.XGBClassifier().fit(X_train, y_train)
            my_nushap = NuSHAP_tabular(model.predict, X_train, feature_names=list(train_df.iloc[:, :-1].columns),
                                       keep_index=True)
            explainer = shap.SamplingExplainer(model=my_nushap.predict_and_collect_samples, data=X_train,
                                               feature_names=my_nushap.data_feature_names)
        else:
